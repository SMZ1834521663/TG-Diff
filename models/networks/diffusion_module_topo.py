...
class DiTBlock(nn.Module):
    def __init__(self, hidden_dim, num_heads, mlp_ratio=4.0):
        ...
        self.mlp_ff = SwiGLUFFN(hidden_dim, int(3/4 * mlp_hidden_dim))
    ...
    def forward(self, e, c): 
        if len(c.shape) < len(e.shape):
            c = c.unsqueeze(1)

        shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = self.adaLN_modulation(c).chunk(6, dim=-1)
        e = e + gate_msa * self.attn(modulate(self.norm1(e),shift_msa, scale_msa)) 
        e = e + gate_mlp * self.mlp_ff(modulate(self.norm2(e), shift_mlp, scale_mlp)) 
        return e
